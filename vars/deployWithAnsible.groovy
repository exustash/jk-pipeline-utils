...
Void call(String deployEnv, String serviceName, String playbookPath) {
    reportBuildStatus("release/pipeline/service.deploy", "Pipeline Task service.deploy is runing", 'PENDING')
    try {
        ...
        reportBuildStatus("release/pipeline/service.deploy", "Pipeline Task service.build has succeeded", 'SUCCESS')

    } catch (err) {
       reportBuildStatus("release/pipeline/service.deploy", "Pipeline Task service.deploy has failed", 'FAILURE')
