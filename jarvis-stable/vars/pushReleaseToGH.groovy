...
Void call(String repositoryName, String packageName, String releaseTag, String commitMsg, String artifactType='tar.gz') {
    reportBuildStatus("release/pipeline/release.push", "Pipeline Task release.push is runing", 'PENDING')
    try {
        ...
        reportBuildStatus("release/pipeline/release.push", "Pipeline Task release.push  has succeeded", 'SUCCESS')

    } catch (err) {
      reportBuildStatus("release/pipeline/release.push", "Pipeline Task release.push  has failed", 'FAILURE')
      error("[ERR!] Pipeline execution error: ${err.message}")
    }
}

Void cleanupGHRelease(String repositoryName, String releaseTag) {
    sh returnStdout: false, script: "github-release delete -u dailymotion -r ${repositoryName} -t ${releaseTag} || true"
}

Void createGHRelease(String repositoryName, String releaseTag, String commitMsg) {
    sh returnStdout: false, script: """github-release release -u dailymotion -r ${repositoryName} -t ${releaseTag} -n ${releaseTag} -d "${commitMsg}" """
}

Void uploadGHRelease(String repositoryName, String packageName, String releaseTag, String artifactType) {
    sh returnStdout: false, script: """github-release upload -u dailymotion -r ${repositoryName} -t ${releaseTag} -n "${packageName}.${artifactType}" -f ${packageName}.${artifactType}"""
}
